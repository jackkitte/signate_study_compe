...
def visualize_for_continuous(df, name_list):
    ...
    for name in generator_for_name:
        df.plot.scatter(x=name, y="y", ax=ax[row_index][column_index], c=Color)
        column_index += 1
...
def visualize_for_category(df, name_list):
    Color = numpy.array([0.5, 0.5, 0.5]).reshape(1, -1)
    row, column = name_list.shape
    ...
    row_index, column_index = 0, 0
    for name in generator_for_name:
        seaborn.boxplot(x=name, y="y", data=df, ax=ax[row_index][column_index])
        column_index += 1
        if column == column_index:
            row_index += 1
            column_index = 0
...
def visualize_for_category_1d(df, name_list):
    Color = numpy.array([0.5, 0.5, 0.5]).reshape(1, -1)
    row = len(name_list)
    ...
    index = 0
    for name in generator_for_name:
        seaborn.boxplot(x=name, y="y", data=df, ax=ax[index])
        index += 1
